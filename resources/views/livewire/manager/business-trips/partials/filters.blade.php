@php
    $filterOptions = [
        ...
        'completed' => ['label' => 'Completed', 'color' => 'bg-blue-100 text-blue-800 hover:bg-blue-200']
    ];
@endphp
...
@foreach($filterOptions as $key => $option)
    <button
        ...
        {{ $option['label'] }}
        @if($filter === $key)
            <span class="ml-2 inline-flex items-center justify-center bg-white bg-opacity-25 rounded-full h-5 w-5 text-xs">
                {{ $counts[$key] ?? 0 }}
            </span>
        @endif
    </button>
