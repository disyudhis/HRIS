    ...
    <div class="space-y-4">
            ...
            <div class="border border-gray-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-lg font-medium text-gray-900">{{ $overtime->user->name }}</h3>
                    ...
                    @if($overtime->status === 'pending')
                        <div class="flex space-x-2">
                            <button wire:click="approve({{ $overtime->id }})" class="px-3 py-1 bg-green-100 text-green-800 rounded-lg text-sm font-medium">
                                Approve
                            </button>
                            <button wire:click="showRejectModal({{ $overtime->id }})" class="px-3 py-1 bg-red-100 text-red-800 rounded-lg text-sm font-medium">
                                Reject
                            </button>
                        </div>
                    @else
                        <span class="px-3 py-1 rounded-lg text-sm font-medium {{ $overtime->status === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                ...
                <div class="mt-4 pt-4 border-t border-gray-100">
                    ...
                    @if($overtime->status !== 'pending')
                        <div class="mt-2 text-sm text-gray-500">
                            <span class="font-medium">{{ ucfirst($overtime->status) }} on:</span>
                            {{ $overtime->approved_at->format('M d, Y h:i A') }}
                        </div>
        ...
        @empty
            <div class="text-center py-8">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900">No overtime requests</h3>
                <p class="mt-1 text-sm text-gray-500">
                    @if($filter === 'pending')
                        There are no pending overtime requests.
                    @elseif($filter === 'approved')
                        There are no approved overtime requests.
                    @elseif($filter === 'rejected')
                        There are no rejected overtime requests.
                    @endif
                </p>
            </div>
        @endforelse
